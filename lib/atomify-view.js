...
export default class AtomifyView {
  ...
  initialize() {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'atomify:turnOnShuffle': () => SpotifyClient.turnOnShuffle()
    }));

    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'atomify:turnOffShuffle': () => SpotifyClient.turnOffShuffle()
    }));

    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'atomify:turnOnRepeat': () => SpotifyClient.turnOnRepeat()
    }));

    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'atomify:turnOffRepeat': () => SpotifyClient.turnOffRepeat()
    }));
